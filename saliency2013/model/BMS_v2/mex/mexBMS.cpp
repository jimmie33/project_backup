...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
	...
	int DILATION_WIDTH_1	=	(rhs[3].toInt()-1)/2;//3: omega_d1
	int DILATION_WIDTH_2	=	(rhs[4].toInt()-1)/2;//11: omega_d2

	float BLUR_STD			=	(float)rhs[5].toDouble();//20: sigma	
	...
	int COLORSPACE			=	rhs[6].toInt();//
	bool WHITENING			=	rhs[7].toInt();
	
	float MAX_DIM			=	-1.0f;
	if (nrhs > 8)
		MAX_DIM				=	(float)rhs[8].toInt();
